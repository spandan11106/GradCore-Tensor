...
bool tensor_l2_loss_grad(Tensor *out, const Tensor *weights, const Tensor *grad,
  ...
  if (reduction == REDUCTION_MEAN) {
    scale = 2.0f / static_cast<float>(weights->size);
  }
...
#pragma omp parallel for
      ...
      for (uint64_t i = 0; i < out->size; i++) {
        ...
        out->storage->data[out->offset + i] = g * scale * w;
      }
    } else {
...
#pragma omp parallel for
      ...
      for (uint64_t i = 0; i < out->size; i++) {
        ...
        out->storage->data[out->offset + i] = final_scale * w;
      }
    }
  } else {
    if (reduction == REDUCTION_NONE) {
      ...
      for (uint64_t i = 0; i < out->size; i++) {
        ...
        out->storage->data[o_idx] = g * scale * w;
      ...
      for (uint64_t i = 0; i < out->size; i++) {
        ...
        out->storage->data[o_idx] = final_scale * w;
