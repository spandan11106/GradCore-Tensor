...
Variable *nll_loss(Arena *arena, Variable *log_probs, Variable *target,
  ...
  tensor_nll_loss(out_data, log_probs->data, target->data, reduction);
  ...
  if (out->requires_grad) {
    ...
    out->parents[0] = {log_probs};

    out->num_saved = 2;
    out->saved_tensors = arena->push_array<Tensor *>(2);
    out->saved_tensors[0] = log_probs->data;
    ...
    out->backward_fn = [](Variable *self, Arena *temp_arena) {
      ...
      tensor_nll_loss_grad(local_grad, self->saved_tensors[0],
