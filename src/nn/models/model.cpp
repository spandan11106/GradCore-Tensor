...
Model::Model(Arena *perm_arena, Arena *graph_arena)
    : perm_arena(perm_arena), graph_arena(graph_arena), learning_rate(0.001f),
      epochs(10), batch_size(32), is_compiled(false), optimizer_ptr(nullptr),
      loss_ptr(nullptr) {
...
void *Model::create_loss() {
  ...
  case LossType::HUBER: {
    HuberLoss *loss = perm_arena->push<HuberLoss>();
    new (loss) HuberLoss(); // Uses the default delta of 1.0f
    return loss;
  }
  default:
...
void Model::compile(OptimizerType optimizer, LossType loss, float lr,
                    uint32_t num_epochs, uint32_t batch_sz) {
...
TrainingStats Model::train(const std::vector<std::vector<float>> &X_train,
  ...
  if (!is_compiled) {
    std::cerr << "Error: Model must be compiled before training. Call "
                 "compile() first."
              << std::endl;
    return empty_stats;
  }
  ...
  data::Dataset *features_dataset =
      data::Dataset::create_2d(perm_arena, X_train);
  ...
  data::DataLoader *dataloader = data::DataLoader::create(
      features_dataset, labels_dataset, batch_size, true, 42);
...
#define TRAIN_WITH_OPTIMIZER(OPT_TYPE)                                         \
  do {                                                                         \
    Trainer<OPT_TYPE, CrossEntropyLoss> trainer(                               \
        sequential, (OPT_TYPE *)optimizer_ptr, (CrossEntropyLoss *)loss_ptr,   \
        graph_arena);                                                          \
  ...
  if (optimizer_type == OptimizerType::ADAM &&
    ...
    Trainer<optim::Adam, CrossEntropyLoss> trainer(
        sequential, (optim::Adam *)optimizer_ptr, (CrossEntropyLoss *)loss_ptr,
        graph_arena);
    return trainer.fit_dataloader(dataloader, epochs, 1);
  } else if (optimizer_type == OptimizerType::SGD &&
             loss_type == LossType::CROSS_ENTROPY) {
    Trainer<optim::SGD, CrossEntropyLoss> trainer(
        sequential, (optim::SGD *)optimizer_ptr, (CrossEntropyLoss *)loss_ptr,
        graph_arena);
    return trainer.fit_dataloader(dataloader, epochs, 1);
  } else if (optimizer_type == OptimizerType::ADAM &&
             loss_type == LossType::MSE) {
    Trainer<optim::Adam, MSELoss> trainer(sequential,
                                          (optim::Adam *)optimizer_ptr,
                                          (MSELoss *)loss_ptr, graph_arena);
    return trainer.fit_dataloader(dataloader, epochs, 1);
  } else if (optimizer_type == OptimizerType::ADAMW &&
             loss_type == LossType::HUBER) {
    Trainer<optim::AdamW, HuberLoss> trainer(
        sequential, (optim::AdamW *)optimizer_ptr, (HuberLoss *)loss_ptr,
        graph_arena);
    return trainer.fit_dataloader(dataloader, epochs, 1);
  } else {
    std::cerr << "Error: Optimizer/Loss combination not yet supported"
              << std::endl;
    return empty_stats;
  }
}
...
float Model::evaluate(const std::vector<std::vector<float>> &X_test,
  ...
  data::Dataset *features_dataset =
      data::Dataset::create_2d(perm_arena, X_test);
  ...
  data::DataLoader *dataloader = data::DataLoader::create(
      features_dataset, labels_dataset, batch_size, false);
...
void Model::summary() const {
  ...
  std::cout << "Total Parameters: " << sequential->num_parameters()
            << std::endl;
  ...
  if (is_compiled) {
    ...
    std::cout
        << "\nStatus: Not compiled. Call compile() to set hyperparameters."
        << std::endl;
  }
  std::cout << "===================" << std::endl;
}
